...
# This prototype saves the working fold files into the /models/training_cache/ folder and saves the final model into the /models/ folder.
...
final_save_path = os.path.join(models_dir, f'defect_classifier_f1_{timestamp}.pth')  # Define final save path for the rolled-up model. Will be used after k-fold and eval
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(full_dataset)):
    ...
    best_val_f1 = 0.0  # Initializes at zero so F1 can increase
    train_patience = 15
    patience_counter = 0
    for epoch in range(100):
        ...
        val_correct = 0
        val_total = 0
        val_preds = []  # Initialize outside the inner loop
        val_labels = []  # Initialize outside the inner loop
        with torch.no_grad():
        ...
        val_f1 = f1_score(val_labels, val_preds, average='weighted')  # Compute F1-score
        ...
        metrics = {"epoch": epoch+1, "train_loss": train_loss, "val_loss": val_loss, "train_acc": train_acc, "val_acc": val_acc, "val_f1": val_f1, "lr": lr}
        metrics_path = os.path.join(training_cache_dir, f"training_metrics_fold_{fold +1}.json")
        with open(metrics_path, "w") as f:
            json.dump(metrics, f)

        print(f'Epoch {epoch+1}/100, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Train Acc: {train_acc:.4f}, Val Acc: {val_acc:.4f}, Val F1: {val_f1:.4f}, LR: {lr:.6f}')
        scheduler.step(val_f1)  # Step based on F1-score
        if val_f1 > best_val_f1:  # F1-based stopping
            best_val_f1 = val_f1
            patience_counter = 0
            best_model_path = os.path.join(training_cache_dir, f'best_model_fold_{fold + 1}_f1.pth')
    ...
    fold_results.append({'fold': fold + 1, 'val_f1': best_val_f1, 'val_loss': val_loss})
...
best_model_path = max(best_models, key=lambda p: fold_results[best_models.index(p)]['val_f1']) # Use max F1 for consistency with early stopping behavior.
...
model.eval()
with torch.no_grad():
    num_batches_to_show = 15 # Show up to 15 batches for variety
